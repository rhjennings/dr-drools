...
/**
 * Returned QueryResults instance for a requested named query. from here you can
 * iterate the returned data, or get a specific row. All the available
 * Declarations used in the query can also be accessed.
 */
public class QueryResults implements Iterable<QueryResult> {
	...
	private class QueryResultsIterator implements Iterator<QueryResult> {
		...
		public QueryResultsIterator(
				final Iterator<QueryRowWithSubruleIndex> iterator) {
		...
		public QueryResult next() {
			return new QueryResult(this.iterator.next(),
					QueryResults.this.workingMemory, QueryResults.this);
		}
